            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    ...
                    @if(auth()->user()->hasRole('admin') || auth()->user()->hasRole('editor')) <!-- Check if the user has the admin or editor role -->
                        <a href="{{ route('dashboard.create') }}" class="btn btn-primary mt-4" title="Tambah Buku">Tambah Buku</a>
                    @endif
                                ...
                                <tr>
                                    ...
                                    @if(auth()->user()->hasRole('admin') || auth()->user()->hasRole('editor')) <!-- Check if the user has the admin or editor role -->
                                        <td class="border px-4 py-2">
                                                <a href="{{ route('dashboard.edit', $book->id) }}" class="btn btn-secondary ml-2" title="Edit Buku">Edit Buku</a>
                                        </td>
                                    @endif
                                    @if(auth()->user()->hasRole('admin')) <!-- Check if the user has the admin role -->
                                        <td class="border px-4 py-2">
                                            <form action="{{ route('dashboard.destroy', $book->id) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus buku ini?');">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" class="btn btn-danger" title="Hapus Buku">Hapus Buku</button>
                                            </form>
                                        </td>
                                    @endif
                                </tr>
